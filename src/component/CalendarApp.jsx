...
const CalendarApp = () => {
  ...
  const validateEvent = () => {
    ...
    if (!newEvent.name.trim()) errors.name = "Name is required.";
    if (!newEvent.email.trim()) {
      errors.email = "Email is required.";
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newEvent.email.trim())) {
    ...
    if (newEvent.start_date && newEvent.end_date && newEvent.end_date < newEvent.start_date) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-900 via-orange-900 to-yellow-900 relative overflow-hidden">
      ...
      <div className="relative z-10 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <EventSubmitForm animations={animations} User={User} />
        </div>
      </div>
